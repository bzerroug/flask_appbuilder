...
def fillGA():
    ...
    for j in range(len(all_dates)-1):
        ...
        site=float(get_TTR_site('97200935', Date1.strftime('%Y-%m-%d'), Date2.strftime('%Y-%m-%d')).get('rows')[0][0])/100.0
        print Date1
        try:
            ttr=GA() 
            ttr.DateDebut = Date1
...
def fillDataNbCom():
    all_dates = [datetime.datetime(2016,1,1)]
    for i in range(442):
        k=i+1
...
def Nombre_de_commandes(date2, date1):
    ...
    orders.delivery_date < date1,\
    orders.delivery_date >= date2)))[0][0])
    
def CA(date2, date1):
    return str(int(list(db.session.query(func.sum(orders.amount))\
    .filter(and_(orders.status == 11, orders.delivery_model == 'LAD',\
    orders.delivery_date < date1,\
    orders.delivery_date >= date2)))[0][0]))+' euros'
...
def fillMeteo():
    ...
    days_from = today.isocalendar()[2]
    monday=today - datetime.timedelta(days_from)
    ...
    .filter(and_(orders.status == 11, orders.delivery_model == 'LAD',\
    func.DATE(orders.delivery_date) <= (monday-datetime.timedelta(1)).date(),\
    func.DATE(orders.delivery_date) >= (monday-datetime.timedelta(7)).date()))
    ...
    .filter(and_(orders.status == 11, orders.delivery_model == 'LAD',\
    func.DATE(orders.delivery_date) <= (monday-datetime.timedelta(days=1, weeks=52)).date(),\
    func.DATE(orders.delivery_date) >= (monday-datetime.timedelta(days=7, weeks=52)).date()))
    ...
    orders.order_date >= (today-datetime.timedelta(4)),\
    orders.order_date <= today-datetime.timedelta(1))).group_by(orders.csc)
    
    myquery22015 = db.session.query(orders.csc, func.count(orders.order_id))\
    .filter(and_(orders.status != 1004,\
    orders.order_date >= (today-datetime.timedelta(days=4, weeks=52)),\
    orders.order_date <= (today-datetime.timedelta(days=1, weeks=52)))).group_by(orders.csc)
    ...
    .filter(and_(orders.status != 1004,\
    func.DATE(orders.order_date) == (today-datetime.timedelta(days=1)).date())).group_by(orders.csc)
    
    myquery32015 = db.session.query(orders.csc, func.count(orders.order_id))\
    .filter(and_(orders.status != 1004,\
    func.DATE(orders.order_date) == (today-datetime.timedelta(days=1, weeks=52)).date())).group_by(orders.csc)    
    ...
    myquery4_appli = db.session.query( func.count(orders.order_id) )\
    .filter(and_(orders.order_date >= (monday-datetime.timedelta(7)),\
    orders.order_date <= monday, orders.modified_order_id.is_(None),\
    or_(orders.device_type==6, orders.device_type==5))).all()
    ...
    N=monday
    N1=monday-datetime.timedelta(7)        
    
    sessions_appli = int(getSessions('97198927', N1.strftime('%Y-%m-%d'), N.strftime('%Y-%m-%d')))
    commandes_appli = int(list(myquery4_appli)[0][0])
    ...
    DF_slot_filling.to_csv('Files/DF_slot_filling.csv', sep=';')
    
    with open('Files/out_kpis.csv', 'wb') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        wr.writerow(out_kpis)
    with open('Files/out_nb_com.csv', 'wb') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        wr.writerow(out_nb_com)
    with open('Files/out_TTR.csv', 'wb') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        wr.writerow(out_TTR)
    with open('Files/out_nb_com_today.csv', 'wb') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
...
def miseAjour():
    ...
    DF=process_compute_slot_filling(6)
    ...
    Date1=(datetime.datetime(2016,03,28)-datetime.timedelta(7)).date()
    Date2=datetime.datetime(2016,03,28).date()
    appli=TTR_appli(Date1, Date2)
...
#        #log.error("Update ViewMenu error: {0}".format(str(e)))
    ...
    for i in range(1,8):    
    	dates.append(dates[0]-datetime.timedelta(i))
...
def daily():
    ...
    DF=process_compute_slot_filling(1)

    try:
        for i in range(len(DF)):
    	    D=datetime.datetime.strptime(DF['Date'][i],'%Y/%m/%d').date()
            if D==datetime.datetime.now() and D.isocalendar()[2]!=6:
		sf=slot_filling()
            	sf.date = D
            	sf.csc = DF['No de magasin'][i]
            	sf.filling = DF['Taux de remplissage'][i]
...
def weekly():
    all_dates=[datetime.datetime.now().date()-datetime.timedelta(7), datetime.datetime.now().date()-datetime.timedelta(1)]
    for j in range(len(all_dates)-1):
        Date1=all_dates[j]
        Date2=all_dates[j+1]
        appli=TTR_appli(Date1, Date2)
